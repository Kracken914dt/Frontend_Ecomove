...
const Estaciones = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const transportesInvalidos = transportesSeleccionados.filter(
        (transporteId) => {
          const estacionAsignada = transportesAsignados[transporteId];
          return (
            estacionAsignada &&
            (!editingStation || estacionAsignada !== editingStation.id)
          );
        }
      );
      ...
      if (transportesInvalidos.length > 0) {
        toast.error(
          "Algunos transportes seleccionados ya están asignados a otras estaciones"
        );
        return;
      }
  ...
  return (
    <div className="space-y-8">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="col-span-2">
                  ...
                  <select
                    multiple
                    {...register("transportes", {
                      required: "Seleccione al menos un transporte",
                    })}
                  ...
                  >
                    {transportes.map((transporte) => {
                      const estacionAsignada =
                        transportesAsignados[transporte.id];
                      const estacionActual = editingStation
                        ? editingStation.id
                        : null;
                      const estaAsignadoAOtraEstacion =
                        estacionAsignada && estacionAsignada !== estacionActual;
                      ...
                      return (
                        <option
                          ...
                          className={
                            estaAsignadoAOtraEstacion ? "text-gray-400" : ""
                          }
                        >
                          {transporte.tipo}
                          {estaAsignadoAOtraEstacion
                            ? " (Asignado a otra estación)"
                            : ""}
                        </option>
                      );
                    })}
      ...
      <div className="relative animate-fade-in">
        <Search
          className="absolute left-3 top-1/2 transform -translate-y-1/2 text-eco-gray-400 transition-all duration-300 group-hover:scale-110"
          size={20}
        />
        <input
          type="text"
              ...
              {filteredEstaciones.map((estacion) => (
                ...
                >
                  ...
                  <div className="space-y-3">
                    ...
                    {estacion.transportes &&
                      estacion.transportes.length > 0 && (
                          ...
                          <div className="flex flex-wrap gap-2">
                            {estacion.transportes.map((transporteId) => {
                              ...
                              return (
                                <span
                                  key={transporteId}
                                  className="px-2 py-1 bg-eco-green-100 text-eco-green-800 rounded-lg text-sm"
                                >
                  ...
                  <div className="mt-6 pt-4 border-t border-eco-gray-200">
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-eco-green-600 to-eco-green-700 text-white py-3 rounded-xl font-medium hover:from-eco-green-700 hover:to-eco-green-800 transition-all duration-200 shadow-lg hover:shadow-xl flex items-center justify-center space-x-2"
                    >
                      <Map size={18} />
